...
class ConflictAnalysis(BaseModel):
    ...
    action_suggested: str = Field(description="수정 제안 (충돌 시에만 작성하되, 반드시 '한국어(Korean)'로만 작성할 것)")
...
llm = ChatOpenAI(
    ...
    api_key="EMPTY", 
    model="Qwen/Qwen2.5-7B-Instruct", # 🚨 Qwen 모델로 변경
    temperature=0.0,
    max_tokens=300 # 🚨 무한 생성 방지용 (필수)
)
...
def analyze_conflicts_node(state: GraphState) -> Dict:
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        "🚨중요: 모든 분석 결과와 제안은 반드시 '한국어(Korean)'로만 작성해야 합니다.\n"
