...
# 🚨 Ollama 대신 OpenAI 호환 클라이언트를 임포트합니다.
...
# =========================================================
CURRENT_ENGINE = "vLLM"                          
CURRENT_MODEL = "EXAONE-3.0-7.8B-Instruct"       
...
# 2. 모델 및 리트리버 설정
# ---------------------------------------------------------
# 🚨 vLLM이 열어둔 8000번 포트를 바라보도록 설정합니다.
llm = ChatOpenAI(
    base_url="http://localhost:8000/v1",
    api_key="EMPTY", # 로컬 서버이므로 아무 키나 넣어도 무방합니다.
    model="LGAI-EXAONE/EXAONE-3.0-7.8B-Instruct",
    temperature=0.0,
    max_tokens=300  # 🚨 추가: 무한 생성 루프 방지용 (매우 중요)
)
...
def analyze_conflicts_node(state: GraphState) -> Dict:
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        "🚨중요: 부연 설명이나 인사말을 절대 하지 말고, 오직 요구된 JSON 형식으로만 결과를 출력한 뒤 즉시 생성을 종료하세요.\n\n" # 🚨 추가된 강력한 지시문
        "기존 콘텐츠: {old_content}\n"
        "신규 규정 팩트: {new_fact}"
    )
...
if __name__ == "__main__":
    # 🚨 vLLM이 8000 포트를 사용 중이므로, API 서버는 8080 포트로 변경해야 충돌이 나지 않습니다.
